	...
	for n in ${N[@]}; do
		...
		then	
			for t in ${T[@]}; do
				...
				IFS=' ' read -r -a arr <<< "$str"
				it=${arr[3]}
				./bin/RKA_seq_max_it_alpha.exe dense 10 $m $n $t $alpha $it >> outputs/omp/RKA_alpha.txt
				echo "seq $m $n $t $it"
				export OMP_NUM_THREADS=$t
				./bin/RKA_max_it_alpha.exe dense 10 $m $n $alpha $it >> outputs/omp/RKA_alpha.txt
				echo "par $m $n $t $it"
				((line++))
			done
			((line++))
