...
# sbatch bash/seq/cg_M_20000.sh

#SBATCH --job-name="cg_M_20000"
#SBATCH --mem-per-cpu=32GB
#SBATCH --output=outputs/progress/cg_M_20000.txt
...
for m in ${M[@]}; do
	for n in ${N[@]}; do
		...
		then	
			...
			((line++))
			str="$(sed "${line}q;d" outputs/seq/cg_it_M_20000.txt)"
			...
			echo "$m $n $it $sum $avge $avgr" >> outputs/seq/cg_M_20000.txt
		fi
	done
done