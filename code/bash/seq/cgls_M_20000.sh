...
# sbatch bash/seq/cgls_M_20000.sh

#SBATCH --job-name="cgls_M_20000"
#SBATCH --mem-per-cpu=32GB
#SBATCH --output=outputs/progress/cgls_M_20000.txt
...
rm outputs/seq/cgls_M_20000.txt
...
for m in ${M[@]}; do
	for n in ${N[@]}; do
		...
		then	
			...
			((line++))
			str="$(sed "${line}q;d" outputs/seq/cgls_it_M_20000.txt)"
			...
			echo "$m $n $it $sum $avge $avgr" >> outputs/seq/cgls_M_20000.txt
		fi
	done
done