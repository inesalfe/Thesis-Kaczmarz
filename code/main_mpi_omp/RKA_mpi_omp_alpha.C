...
// Cada thread é responsável por uma iteração.
// Depois faz-se a média enter threads
// Depois faz-se a média entre processadores

// mpic++ -o bin/RKA_mpi_omp_max_it.exe main/RKA_mpi_omp_max_it.C
// mpirun -np 2 ./bin/RKA_mpi_omp_max_it.exe dense 1 1E-10 2000 100 20
...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'mpirun -np <nproc> ./bin/RKA_mpi_omp_alpha.exe <data_set> <n_runs> <eps> <M> <N> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	#pragma omp parallel private(line, scale, t_id, it)
	{
		...
		for(int run = 0; run < n_runs; run++) {
			...
			while(!solution_found) {
				...
				#pragma omp critical
					for (int i = 0; i < N; i++)
						x_k[i] += (x_prev[i] + scale * A[line][i])/(np*num_threads);
				#pragma omp barrier
				#pragma omp single
