...
int main (int argc, char *argv[]) {
	...
	if(argc != 6) {
		...
		cout << "'./bin/CK_seq_error.exe <data_set> <M> <N> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	int M = atoi(argv[2]);
	int N = atoi(argv[3]);
	long long max_it_stop = atoll(argv[4]);
	int step_save = atoi(argv[5]);
	...
	string filename_error = "errors/omp/CK_error_" + to_string(M) + "_" + to_string(N) + ".txt";
	string filename_res = "errors/omp/CK_res_" + to_string(M) + "_" + to_string(N) + ".txt";
