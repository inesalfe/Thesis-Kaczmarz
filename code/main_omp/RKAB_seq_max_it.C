...
int main (int argc, char *argv[]) {
	...
	if(argc != 9) {
		...
		cout << "'./bin/RKAB_seq_max_it.exe <data_set> <n_runs> <M> <N> <blocks> <it_per_thread> <max_it_stop> <block_repeat>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKAB_repeat_seq_max_it.exe <data_set> <n_runs> <M> <N> <blocks> <it_per_thread> <max_it_stop> <block_repeat>'" << endl;
		exit(1);
	}
