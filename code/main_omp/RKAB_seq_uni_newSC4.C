...
int main (int argc, char *argv[]) {
	...
	if(argc != 10) {
		...
		cout << "'./bin/RKAB_seq_uni_newSC4.exe <data_set> <n_runs> <eps> <M> <N> <blocks> <it_per_thread> <max_it_stop> <block_repeat>'" << endl;
		exit(1);
	}
			...
			for (int i = 0; i < blocks; i++) {
				...
				for (int n = 0; n < block_repeat; n++) {
					for (int k = 0; k < it_per_thread; k++) {
						line = BLOCK_LOW(i, blocks, M) + (it*it_per_thread + k)%BLOCK_SIZE(i, blocks, M);
