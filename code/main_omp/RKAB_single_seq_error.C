...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKAB_single_seq_error.exe <data_set> <M> <N> <threads> <it_per_thread> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/omp/RKAB_single_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(threads) + ".txt";
	string filename_res = "errors/omp/RKAB_single_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(threads) + ".txt";
