...
int main (int argc, char *argv[]) {
	...
	if(argc != 10) {
		...
		cout << "'./bin/RKAB_single_seq_uni_newSC2_alpha.exe <data_set> <n_runs> <eps> <M> <N> <threads> <it_per_thread> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
		...
		while(!solution_found) {
			...
			for (int i = 0; i < threads; i++) {
				...
				for (int k = 0; k < it_per_thread; k++) {
					line = samp_line[BLOCK_LOW(i, threads, M) + (it*it_per_thread + k)%BLOCK_SIZE(i, threads, M)];
				...
				// line = samp_line[BLOCK_LOW(i, threads, M) + (it*it_per_thread)%BLOCK_SIZE(i, threads, M)];
				...
				// 	}
				// }
				// line = samp_line[BLOCK_LOW(i, threads, M) + (it*it_per_thread + it_per_thread-1)%BLOCK_SIZE(i, threads, M)];
				// scale = (b[line]-dotProduct(A[line], x_k_thread, N))/sqrNorm_line[line];
				// for (int j = 0; j < N; j++) {
