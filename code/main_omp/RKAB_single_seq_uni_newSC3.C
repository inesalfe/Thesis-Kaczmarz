...
int main (int argc, char *argv[]) {
	...
	if(argc != 9) {
		...
		cout << "'./bin/RKAB_single_seq_uni_newSC3.exe <data_set> <n_runs> <eps> <M> <N> <threads> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKAB_single_seq_uni_newSC3.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
