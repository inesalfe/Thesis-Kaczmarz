...
int main (int argc, char *argv[]) {
	...
	if(argc != 10) {
		...
		cout << "'./bin/RKAB_single_seq_uni_newSC4_alpha.exe <data_set> <n_runs> <eps> <M> <N> <threads> <it_per_thread> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	long long max_it_stop = atoll(argv[9]);
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(!solution_found) {
			...
			for (int i = 0; i < threads; i++) {
				...
				// line = BLOCK_LOW(i, threads, M) + (it*it_per_thread)%BLOCK_SIZE(i, threads, M);
				// scale = (b_shuffle[line]-dotProduct(A_shuffle[line], x_prev, N))/sqrNorm_line_shuffle[line];
				// for (int j = 0; j < N; j++) {
				// 	x_k_thread[j] = x_prev[j] + scale * A_shuffle[line][j];
				// }
				// for (int k = 1; k < it_per_thread-1; k++) {
				// 	line = BLOCK_LOW(i, threads, M) + (it*it_per_thread + k)%BLOCK_SIZE(i, threads, M);
				// 	scale = (b_shuffle[line]-dotProduct(A_shuffle[line], x_k_thread, N))/sqrNorm_line_shuffle[line];
				// 	for (int j = 0; j < N; j++) {
				// 		x_k_thread[j] += scale * A_shuffle[line][j];
				// 	}
				// }
				// line = BLOCK_LOW(i, threads, M) + (it*it_per_thread + it_per_thread-1)%BLOCK_SIZE(i, threads, M);
				// scale = (b_shuffle[line]-dotProduct(A_shuffle[line], x_k_thread, N))/sqrNorm_line_shuffle[line];
				// for (int j = 0; j < N; j++) {
				// 	x_k_thread[j] += scale * A_shuffle[line][j] - x_prev[j];
				// }
				// for (int j = 0; j < N; j++) {
				// 	x_k[j] += alpha * x_k_thread[j]/threads;
				// }
			}
			if (it%max_it_stop == 0) {
