...
int main (int argc, char *argv[]) {
	...
	if(argc != 10) {
		...
		cout << "'./bin/RKAB_single_seq_uni_newSC_alpha.exe <data_set> <n_runs> <eps> <M> <N> <threads> <it_per_thread> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(!solution_found) {
			...
			for (int i = 0; i < threads; i++) {
				...
				// 	}
				// }
				// line = samp_line[(block_begin+it_per_thread-1)%M];
				// scale = (b[line]-dotProduct(A[line], x_k_thread, N))/sqrNorm_line[line];
				// for (int j = 0; j < N; j++) {
