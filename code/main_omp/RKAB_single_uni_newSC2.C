...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKAB_single_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKAB_single_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id, x_k_thread) firstprivate(it)
		{
			...
			while(!solution_found) {
				it++;
				#pragma omp barrier
				line = samp_line[BLOCK_LOW(t_id, num_threads, M) + (it*it_per_thread)%BLOCK_SIZE(t_id, num_threads, M)];
