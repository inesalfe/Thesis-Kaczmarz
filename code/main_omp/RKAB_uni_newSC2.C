...
int main (int argc, char *argv[]) {
	...
	if(argc != 10) {
		...
		cout << "'./bin/RKAB_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop> <blocks> <block_repeat>'" << endl;
		exit(1);
	}
		...
		#pragma omp parallel private(line, scale, t_id, x_k_thread, x_k_block) firstprivate(it)
		{
			...
			while(!solution_found) {
				...
				for (int block_it = 0; block_it < size_blocks; block_it++) {
					line = samp_line[BLOCK_LOW(low_block_id+block_it, blocks, M) + (it*it_per_thread)%BLOCK_SIZE(low_block_id+block_it, blocks, M)];
					scale = (b[line]-dotProduct(A[line], x_k, N))/sqrNorm_line[line];
					for (int j = 0; j < N; j++) {
						x_k_block[j] = x_k[j] + scale * A[line][j];
					}
					for (int i = 1; i < it_per_thread; i++) {
						line = samp_line[BLOCK_LOW(low_block_id+block_it, blocks, M) + (it*it_per_thread+i)%BLOCK_SIZE(low_block_id+block_it, blocks, M)];
