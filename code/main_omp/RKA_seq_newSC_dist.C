...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKA_seq_newSC_dist.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKA_seq_newSC_dist.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
