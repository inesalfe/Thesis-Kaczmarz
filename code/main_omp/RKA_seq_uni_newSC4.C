...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKA_seq_uni_newSC4.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKA_seq_uni_newSC4.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(!solution_found) {
			...
			for (int i = 0; i < it_per_thread; i++) {
				...
				for (int j = 0; j < N; j++) {
					x_k[j] += (scale * A_shuffle[line][j])/it_per_thread;
				}
			}
			if (it%max_it_stop == 0) {
