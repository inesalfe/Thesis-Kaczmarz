...
int main (int argc, char *argv[]) {
	...
	if(argc != 9) {
		...
		cout << "'./bin/RKA_seq_uni_newSC4_alpha.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(!solution_found) {
			...
			for (int i = 0; i < it_per_thread; i++) {
				...
				scale = alpha * (b_shuffle[line]-dotProduct(A_shuffle[line], x_prev, N))/sqrNorm_line_shuffle[line];
				for (int j = 0; j < N; j++) {
