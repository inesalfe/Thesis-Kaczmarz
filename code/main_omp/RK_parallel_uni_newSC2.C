...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'./bin/RK_parallel_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
		...
		#pragma omp parallel firstprivate(it)
		{
			while(!solution_found) {
				...
				#pragma omp for reduction(+:dot_p)
					for (int i = 0; i < N; i++)
						dot_p += A_shuffle[line][i]*x_k[i];
				#pragma omp single
					scale = (b_shuffle[line]-dot_p)/sqrNorm_line_shuffle[line];
				#pragma omp for
					for (int i = 0; i < N; i++) {
