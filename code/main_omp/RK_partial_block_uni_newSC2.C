...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RK_partial_block_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RK_partial_block_uni_newSC2.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
		...
		#pragma omp parallel private(line, scale, t_id) firstprivate(it)
		{
			...
			while(!solution_found) {
				...
				for (int i = 0; i < it_per_thread; i++) {
					line = samp_line[BLOCK_LOW(t_id, num_threads, M) + (it*it_per_thread + i)%BLOCK_SIZE(t_id, num_threads, M)];
					scale = (b[line]-dotProduct(A[line], x_k, N))/sqrNorm_line[line];
					for (int j = BLOCK_LOW(t_id, num_threads, N); j <= BLOCK_HIGH(t_id, num_threads, N); j++)
						x_k[j] += scale * A[line][j];
				}
				if (it%max_it_stop == 0) {
					#pragma omp single
						norm_dif = 0;
