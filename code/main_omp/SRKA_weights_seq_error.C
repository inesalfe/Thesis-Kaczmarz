...
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 1 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 2 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 4 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 8 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 20 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 50 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 100 30000 1000
// ./bin/SRKA_weights_seq_error.exe ls_dense 4000 1000 1000 30000 1000

int main (int argc, char *argv[]) {

	if(argc != 7) {
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/SRKA_weights_seq_error.exe <data_set> <M> <N> <threads> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	while(it < max_it_stop) {
		...
		for (int i = 0; i < threads; i++) {
			...
			for (int j = 0; j < N; j++) {
				x_k[j] += (x_prev[j] + scale * A[line][j])/threads;
			}
		}
		if (it%step_save == 0) {
	...
	string filename_error = "errors/omp/SRKA_weights_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + ".txt";
	string filename_res = "errors/omp/SRKA_weights_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + ".txt";
