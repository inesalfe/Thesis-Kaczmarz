...
int main (int argc, char *argv[]) {
	...
	if(argc != 6) {
		...
		cout << "'./bin/real_csr.exe <method> <n_runs> <eps> <M> <N>'" << endl;
		exit(1);
	}
	...
	string filename_values = "../data/sparse_real/values_csr_" + to_string(M) + "_" + to_string(N) + ".bin";
	...
	importCSRMatrixSparseBIN(M, N, NNZ, filename_row_idx, filename_cols, filename_values, row_idx, cols, values_csr);
